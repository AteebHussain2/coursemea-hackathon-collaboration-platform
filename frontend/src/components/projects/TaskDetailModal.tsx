...
const TaskDetailModal: React.FC<TaskDetailModalProps> = ({
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-white dark:bg-gray-900 rounded-[2.5rem] shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col animate-in zoom-in duration-300 border border-gray-100 dark:border-gray-800">
                {/* Header */}
                <div className="p-8 border-b border-gray-50 dark:border-gray-800 flex items-start justify-between">
                ...
                {isLoading ? (
                    ...
                    <div className="flex-1 overflow-y-auto flex">
                        {/* Left Side: Details */}
                        <div className="flex-[1.5] p-8 border-r border-gray-50 dark:border-gray-800 space-y-8">
                            <div>
                            ...
                            <div className="pt-8 border-t border-gray-50 dark:border-gray-800">
                                ...
                                <div className="flex items-center space-x-3 p-4 bg-gray-50 dark:bg-gray-800/50 rounded-3xl border border-transparent dark:border-gray-800">
                                    {task?.assignedTo ? (
                                        <>
                                            <div className="h-10 w-10 rounded-full bg-indigo-100 dark:bg-indigo-900/40 flex items-center justify-center text-indigo-600 dark:text-indigo-400 font-bold overflow-hidden shadow-sm">
                                            ...
                                            <div>
                                                <div className="text-sm font-bold text-gray-900 dark:text-white">{task.assignedTo.name}</div>
                                                <div className="text-xs text-gray-400 dark:text-gray-500">{task.assignedTo.email}</div>
                                            </div>
                                        </>
                                    ) : (
                                ...
                                <div className="space-y-2">
                                    {task?.attachments && task.attachments.length > 0 ? (
                                        task.attachments.map((file: any, idx: number) => (
                                            <div key={idx} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-800 rounded-2xl group hover:bg-gray-100 dark:hover:bg-gray-750 transition-all border border-transparent dark:border-gray-700">
                                                ...
                                                <a
                                                    ...
                                                    className="p-1.5 bg-white dark:bg-gray-700 rounded-lg shadow-sm text-gray-400 hover:text-indigo-600 transition-all opacity-0 group-hover:opacity-100"
                                                >
                                        ...
                                        <div className="text-center py-4 text-xs text-gray-400 italic bg-gray-50/50 dark:bg-gray-800/30 rounded-2xl border border-dashed border-gray-100 dark:border-gray-800">
                                            No attachments yet.
                                        </div>
                                    )}
                                </div>
                        ...
                        {/* Right Side: Comments */}
                        <div className="flex-1 bg-gray-50/50 dark:bg-gray-950/20 p-8 flex flex-col border-l border-gray-50 dark:border-gray-800">
                                    ...
                                    comments.map(comment => (
                                        <div key={comment._id} className="group">
                                            <div className="flex items-start space-x-3">
                                                <div className="h-8 w-8 rounded-full bg-white dark:bg-gray-800 shadow-sm flex items-center justify-center text-[10px] font-bold text-indigo-600 dark:text-indigo-400 border border-gray-100 dark:border-gray-700 overflow-hidden">
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    <div className="flex items-center justify-between mb-1">
                                                        <span className="text-xs font-bold text-gray-900 dark:text-white truncate">{comment.userId.name}</span>
                                                        <span className="text-[10px] text-gray-400">{new Date(comment.createdAt).toLocaleDateString()}</span>
                                                    </div>
                                                    <div className="bg-white dark:bg-gray-800 p-3 rounded-2xl rounded-tl-none border border-gray-100 dark:border-gray-700 shadow-sm text-xs text-gray-600 dark:text-gray-400 leading-relaxed wrap-break-word relative group">
                                                        {comment.content}
                                                        {comment.userId._id === user?._id && (
                                                            <button
                                                                onClick={() => handleDeleteComment(comment._id)}
                                                                className="absolute -right-2 -top-2 p-1 bg-red-50 dark:bg-red-900/40 text-red-400 rounded-full opacity-0 group-hover:opacity-100 transition-opacity hover:text-red-600 border border-red-100 dark:border-red-900/50"
                                                            >
                            ...
                            <form onSubmit={handleAddComment} className="relative">
                                <textarea
                                    ...
                                    className="w-full bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-3xl p-4 pr-12 text-xs focus:ring-2 focus:ring-indigo-500 outline-none shadow-sm dark:shadow-none resize-none text-gray-900 dark:text-white"
                                    rows={3}
                                />
                                <button
                                    type="submit"
                                    disabled={isSending || !newComment.trim()}
                                    className="absolute right-3 bottom-4 p-2 bg-indigo-600 text-white rounded-xl shadow-lg shadow-indigo-100 dark:shadow-none hover:bg-indigo-700 transition-all active:scale-95 disabled:opacity-50"
                                >
